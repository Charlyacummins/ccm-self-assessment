...
export function ReviewersContent({ templateOptions }: { templateOptions: TemplateOption[] }) {
  ...
  const handleCsvFile = (file: File) => {
    ...
    reader.onload = (e) => {
      ...
      const startIdx = firstCols[0] && !firstCols[0].includes("@") && !/^\d/.test(firstCols[0]) && firstCols[1] && !firstCols[1].includes("@") ? 1 : 0;
    ...
    for (const row of csvRows) {
      try {
        const res = await fetch("/api/corp-admin/cohort-invites", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ cohortId, corporationId, email: row.email, name: row.name, role: "reviewer", resend: false }),
        });
      ...
      <Card>
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between">
            <CardTitle className="text-base text-[#004070]">Invite Reviewer</CardTitle>
