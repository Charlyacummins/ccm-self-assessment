...
export async function POST(req: Request) {
  ...
  const { admin_external_id, cohort_id, cohort_external_id } = data;

  if (!admin_external_id || (!cohort_id && !cohort_external_id)) {
    return NextResponse.json(
      { ok: false, error: "Missing required fields: admin_external_id and one of cohort_id or cohort_external_id" },
      { status: 400 }
    );
  }
