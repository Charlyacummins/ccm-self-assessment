...
export async function GET(req: Request) {
  ...
  const { data: scoreRows } = assessmentIds.length
    ? await supabase
        .from("assessment_skill_scores")
        .select("final_score, template_skill_id, template_skills(name, skill_group_id, max_points)")
        .in("assessment_id", assessmentIds)
    : { data: [] as Array<Record<string, unknown>> };

  const groupAgg = new Map<string, { total: number; possible: number }>();
  const skillAgg = new Map<
    string,
    { name: string; groupId: string; maxPoints: number; total: number; count: number }
  >();
  ...
  for (const row of scoreRows ?? []) {
    ...
    if (!groupId || !templateSkillId) continue;

    const groupCurrent = groupAgg.get(groupId) ?? { total: 0, possible: 0 };
    groupCurrent.total += finalScore;
    groupCurrent.possible += maxPoints;
    groupAgg.set(groupId, groupCurrent);
  ...
  const skillScores: SkillScore[] = [...skillAgg.entries()].map(([templateSkillId, agg]) => ({
    ...
    rawScore: agg.count > 0 ? agg.total / agg.count : 0,
  }));
